...
import mu from './utils.js'
import Request from './request.js'
import Response from './response.js'

import pl from '@grpc/proto-loader'

const REMOVE_PROPS = ['grpc', 'servers', 'load', 'proto', 'data']
 ...
 */
export default class Mali extends Emitter {
  /**
  ...
  addService(path, name, options) {
    const load =
      _.isString(path) || (_.isObject(path) && path.root && path.file)
    ...
    this.data = Object.assign({}, this.data, picked)
    ...
    if (_.isFunction(service)) {
      ...
      _.forOwn(this.data, (sd, sn) => {
        if (_.isFunction(name)) {
          // make sure name is given and is also a function
        ...
        _.forOwn(service, (def, serviceName) => {
          _.forOwn(def, (mw, mwName) => {
            if (_.isFunction(mw)) {
              ...
              throw new TypeError(
                `Handler for ${mwName} is not a function or array`,
              )
            }
          })
        })
      ...
      if (sd.handlers[methodName]) {
        throw new Error(
          `Handler for ${name} already defined for service ${serviceName}`,
        )
      }
  ...
  close() {
    return Promise.all(
      this.servers.map(({ server }) => server.tryShutdownAsync()),
    )
  }
  ...
  _createContext(call, descriptor) {
    const type =
      mu.getCallTypeFromCall(call) || mu.getCallTypeFromDescriptor(descriptor)

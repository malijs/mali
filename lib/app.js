...
class Mali extends Emitter {
  ...
  addService (path, name, options) {
    ...
    const data = pu.getServiceDefintions(proto)
   ...
   * If <code>service</code> and <code>name</code> are given applies fns for that calls under that service.
        ...
        _.forOwn(service, (val, key) => {
          ...
          if (serviceName) {
            ...
            if (serviceName && methodName) {
              ...
              throw new TypeError(`Unknown method call(s)`)
            }
          }
        })
  ...
  start (port, creds, options) {
    ...
    console.log('===========')
    console.dir(this.data, { depth: 3, colors: true })
    console.log('===========')
